...
checkDuplicateEmail:function(email,res){
      ...
      return new Promise(function(resolve,reject){
           con.query('SELECT * from driver WHERE email=?',email,function(err,rows,fields){
                  if(err){
                    throw err;
                  }
                  resolve(rows);
...
///////////////////////////////CHECK DUPLICATE PHONE////////////////////////////
  ...
  checkDuplicatePhone:function (phone,res){
        ...
        return new Promise(function(resolve,reject){
             con.query('SELECT * from driver WHERE phone=?',phone,function(err,rows,fields){
                    if(err){
                      throw err;
                    }
                    resolve(rows);
...
//////////////////////////CREATE NEW USER///////////////////////////////////////
         ...
         signup:async function(data){
             ...
             let que=`INSERT INTO driver SET ?`;
       ...
       getDriver:async (data)=>{
                try{
                 ...
                 let fetched_data= await  new Promise(function(resolve,reject){
                      con.query('SELECT * from driver WHERE driver_id=?',data,function(err,rows,fields){
                             if(err){
                               throw err;
                             }
                             resolve(rows);
       ...
       putUser:async function(data){
           try{
               ...
               //console.log(a);
                ...
                const updated_data= await new Promise(function(resolve,reject){
                     console.log(`not here`);

                     con.query('UPDATE driver SET email=?,first_name=?,last_name=?,modified_at=?,password=? WHERE driver_id=?',[data.payload.email,data.payload.first_name,data.payload.last_name,modifiedDate,hash,t.id],function(err,rows,fields){
                            if(err){
                              throw err;
                            }
...
///////////////////////////////LOGIN////////////////////////////////////////////
 login:async(data)=>{
  ...
  try{
         ...
         let fetched_data= await  new Promise(function(resolve,reject){
                  con.query('SELECT password,driver_id from driver WHERE email=?',data.email,function(err,rows,fields){
                         if(err){
                           throw err;
                         }
                         resolve(rows);
            ...
            console.log('Entered password',data.password);
             ...
             if( !bcrypt.compareSync(data.password,fetched_data[0].password) ){
                throw exc.invalidPassword;
                   ...
                   console.log("pehle");
                   const token=jwt.sign({id:fetched_data[0].driver_id},privateKey);
                   console.log(token);
...
verifyOTP: async function(req_otp,id_otp){
    ...
    //UPDATE customer SET email=?,first_name=?,last_name=?,modified_at=?,password=? WHERE id=?',[data.payload.email,data.payload.first_name,data.payload.last_name,modifiedDate,hash,t.id],function(err,rows,fields)
                   con.query('UPDATE driver SET is_verified=? WHERE driver_id=?',[true,id_otp.id],function(err,rows,fields){
                          if(err){
                            throw err;
                          }
                          resolve(rows);
...
isAlreadyVerified:async function(id){
          try{
           ...
           console.log('id in isAlreadyVerified',id);
            const is_verified= await new Promise(function(resolve,reject){
                 con.query('SELECT is_verified from driver WHERE driver_id=?',id,function(err,rows,fields){
                        if(err){
                          throw err;
                        }
                        resolve(rows);
...
isAlreadyVerifiedByMail:async function(email){
          try{
            ...
            const is_verified= await new Promise(function(resolve,reject){
                 con.query('SELECT is_verified from driver WHERE email=?',email,function(err,rows,fields){
                        if(err){
                          throw err;
                        }
                        resolve(rows);
